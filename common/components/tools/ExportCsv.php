...
class ExportCsv {
    ...
    public static $filePath='@app/runtime/export';
    ...
    public static function exportQuery($parameter,$success=null){
        ...
        try{
            ...
            $userKey=md5(serialize(self::getUserCacheKey()));
            $filePath='EP'.date("Y").'/'.$userKey;

            $parameter['target']=self::exporPath($parameter['name'],$userKey,$filePath).'/';
            $status=self::limitExport();
            if( $status['status']){
                static::beforExport($userKey,$parameter['target']);
                self::export($parameter,$success);
                if(file_exists(\yii::getAlias(static::$filePath.'/'.$filePath.'/'.$parameter['name']))){
                    $status=['status'=>true,'msg'=>'导出成功','url'=>'/assets/'.$filePath.'/'.$parameter['name']];
                }else{
                    $status=['status'=>false,'msg'=>'导出失败'];
                }
            }
        }catch (Exception $e){
            $status=['status'=>false,'msg'=>'导出失败'];
        }
        static::afterExport($status);
        return $status;
    }
    ...
    public static function exporPath($filename,$codition,$filePath){
        $path=\yii::getAlias(static::$filePath.'/'.$filePath);
     ...
     * @param $filePath
     */
    public static function beforExport($userKey,$filePath){
        //删除 该目录下的所有文件
        $files=FileHelper::findFiles($filePath);
        if(StringHelper::basename($filePath)!=$userKey){
            return false;
        }
     ...
     * 导出结束后的操作
     */
    public static function afterExport($status){
    ...
    public static function limitExport(){
        ...
        if(empty($userCache)){
            ...
            if( isset($userCache['expire']) && $userCache['expire']>time()){
                if((time()-$userCache['export_time'])<30){
                    return ['status'=>false,'msg'=>'您的导出太频繁，请在'.(30-(time()-$userCache['export_time'])).'s后导出'];
                }
                if($userCache['export_count']>50){
    ...
    public static function deal($row,$parameter){
         ...
         * 处理时间
         */
        $timeInclude=['add_time','edit_time','pay_time','finish_time','end_time','start_time','end_time'];
        ...
        foreach ($timeKeys as $timeKey){
            if($row[$timeKey]){
                if(strlen($row[$timeKey])>11){
                    if($row[$timeKey] == '0000-00-00 00:00:00'){
                        $row[$timeKey]=null;
                    }
                }else{
