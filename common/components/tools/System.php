...
class System
{
    ...
    function sys_linux()
    {
        ...
        $res['memPercent'] = (floatval($res['memTotal'])!=0)?round($res['memUsed']/($res['memTotal']?:1)*100,2):0;

        $res['memRealUsed'] = $res['memTotal'] - $res['memFree'] - $res['memCached'] - $res['memBuffers']; //真实内存使用
        $res['memRealFree'] = $res['memTotal'] - $res['memRealUsed']; //真实空闲
        $res['memRealPercent'] = (floatval($res['memTotal'])!=0)?round($res['memRealUsed']/($res['memTotal']?:1)*100,2):0; //真实内存使用率

        $res['memCachedPercent'] = (floatval($res['memCached'])!=0)?round($res['memCached']/($res['memTotal']?:1)*100,2):0; //Cached内存使用率

        $res['swapTotal'] = round($buf[4][0]/1024, 2);
        $res['swapFree'] = round($buf[5][0]/1024, 2);
        $res['swapUsed'] = round($res['swapTotal']-$res['swapFree'], 2);
        $res['swapPercent'] = (floatval($res['swapTotal'])!=0)?round($res['swapUsed']/($res['swapTotal']?:1)*100,2):0;
...
//        return $data['cpu0']['user']."%us,  ".$data['cpu0']['sys']."%sy,  ".$data['cpu0']['nice']."%ni, ".$data['cpu0']['idle']."%id,  ".$data['cpu0']['iowait']."%wa,  ".$data['cpu0']['irq']."%irq,  ".$data['cpu0']['softirq']."%softirq";
    ...
    public static function getAll(){
        ...
        $df=System::getDf();
        if(is_array($df)){
            $data['df']=['dtTotal'=>$df['dt'],'dfFree'=>$df['df'],'dfPercent'=>round((($df['dt']-$df['df']))/(($df['dt']?:1)-0)*100,2),'dfUsed'=>($df['dt']-$df['df'])];
        }
        ...
        if(is_array($mem)){
            ...
            $data['MemRealPercent']=round($data['MemRealUsed']/($data['MemTotal']?:1)*100,2);
