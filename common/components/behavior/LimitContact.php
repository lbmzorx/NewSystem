 ...
 */
class LimitContact extends  Behavior
{
    const TAG_CONTACT='CONTACT_TAG_TRAN';

    public $category='app';
    public $maxCount=10;
    public $spaceBetweenMail=60; //10 minute

    public $attribute='subject';
    /**
    ...
    {
        return [
            ContactEvent::EVENT_BEFORE_CONTACT=>'beforeContact',
            ContactEvent::EVENT_SUCCESS_CONTACT=>'failedContact',
            ContactEvent::EVENT_FAILED_CONTACT=>'successContact',
            ContactEvent::ENENT_AFTER_CONTACT=>'afterContact',
    ...
    public function beforeContact($event){
        $model=$event->sender;
        $key=[$this->getIpKey(),self::TAG_CONTACT];
        if($msg=Yii::$app->cache->get($key)&&isset($msg['count'])&&isset($msg['expire'])&& $msg['expire']>time()){
            if( $msg['count']<=$this->maxErrorCount ){
                ...
                $msg=yii::t($this->category,'We have send more than {count} Contacts,please don\'t register again!',['count'=>$this->maxCount]);
                \yii::$app->getSession()->setFlash('error',$msg);
                /**
                 * @var $model \yii\base\Model
                 */
                $model->addError($this->attribute,$msg);
                $msg='IP:'.\yii::$app->request->userIP.' key='.serialize($key).' Event=Contact over Sand , More then '.$this->maxCount.'.';
    ...
    public function successContact($event){
        $key=[$this->getIpKey(),self::TAG_CONTACT];
    ...
    public function failedContact($event){

    }

    public function afterContact($event){
